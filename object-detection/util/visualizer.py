...
class COCOVisualizer():
    ...
    def visualize(self, img, tgt, id2path,caption=None, dpi=120, savedir=None, show_in_console=True):
        """
        img: tensor(3, H, W)
...
#             [bbox_x, bbox_y, bbox_w, bbox_h] = unnormbbox.tolist()
#             boxes.append([bbox_x, bbox_y, bbox_w, bbox_h])
#         img_path = id2path[int(tgt["image_id"])]
...
#             if label_y < 0:  # 如果标签位置超出图像顶部
#                 label_y = int(y_min) + 20  # 移到框内或下方
#
#             # 绘制类别标签
#             cv2.putText(image, category_name, (label_x, label_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
#
#
#
#         if savedir is not None:
...
#             print("savename: {}".format(savename))
#             os.makedirs(os.path.dirname(savename), exist_ok=True)
#             cv2.imwrite(savename, cv2.cvtColor(image, cv2.COLOR_RGB2BGR))  # 保存图片
