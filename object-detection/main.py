...
def main(args):
    ...
    if (not args.resume) and args.pretrain_model_path:
        ...
        list1 = []
        list2 = []
        test1=model_without_ddp.state_dict()
        for key in model_without_ddp.state_dict().keys():
            list1.append(key)
        ...
        if args.use_ema:
            if 'ema_model' in checkpoint:
                ...
                ema_m = ModelEma(model, args.ema_decay)
    dataset_val_remain = build_remain(image_set="val", args=args)
    data_loader_val_remain = DataLoader(dataset_val_remain, 1,
                                        drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers)
    dataset_val_forget = build_forget(image_set="val", args=args)
    data_loader_val_forget = DataLoader(dataset_val_forget, 1,
                                        drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers)
    if args.lora_rank > 0:
    ...
    if args.eval:
        ...
        # data_loader_val_learn = DataLoader(dataset_val_learn, 1,
        #                                    drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers)
