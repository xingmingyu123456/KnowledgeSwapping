...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        _bbox_embed = MLP(hidden_dim, hidden_dim, 4, 3,lora_rank=0,lora_pos="NO")
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        out['dn_meta'] = dn_meta
        if self.lora_rank>0:
            structure_loss = self._calculate_structure_loss()
            return out,structure_loss
        else:
            return out,None

    def _calculate_structure_loss(self):
        model_without_ddp = self  # 假设模型未被包装为 DDP 或 DP
        ...
        learnable_params_name = [
            name for name, param in model_without_ddp.named_parameters()
        ...
        for group_item in group_layers:
            group_param = [
                params_dict.get(item) if item in learnable_params_name else None
...
class SetNewCriterion(nn.Module):
    def __init__(self, num_classes, matcher, weight_dict, focal_alpha, losses, forget_classes=None):
        super().__init__()
        self.num_classes = num_classes
        self.matcher = matcher
        self.weight_dict = weight_dict
        self.losses = losses
        self.focal_alpha = focal_alpha
        self.forget_classes = forget_classes if forget_classes is not None else []

    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        """Modified classification loss to handle forgetting"""
        assert 'pred_logits' in outputs
        src_logits = outputs['pred_logits']
        ...
        # 创建遗忘类别的mask
        forget_mask = torch.zeros_like(target_classes_onehot, dtype=torch.bool)
        for c in self.forget_classes:
            forget_mask[:, :, c] = 1

        # 分别计算正常类别和遗忘类别的损失
        normal_loss = sigmoid_focal_loss(
            src_logits[~forget_mask],
            target_classes_onehot[~forget_mask],
            num_boxes,
        ...
        # 对遗忘类别使用较小的负权重
        forget_loss = -0.2 * sigmoid_focal_loss(
            src_logits[forget_mask],
            target_classes_onehot[forget_mask],
            num_boxes,
            alpha=self.focal_alpha,
        ...
        loss_ce = normal_loss + forget_loss
        losses = {'loss_ce': loss_ce}
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        """Modified box loss to maintain localization ability"""
        assert 'pred_boxes' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_boxes = outputs['pred_boxes'][idx]
        target_boxes = torch.cat([t['boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0)
        ...
        # 创建遗忘类别的mask
        forget_mask = torch.tensor([c in self.forget_classes for c in target_classes],
