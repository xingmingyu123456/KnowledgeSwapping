    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        if deformable_encoder:
            encoder_layer = DeformableTransformerEncoderLayer(d_model, dim_feedforward,
                                                          dropout, activation,
                                                          num_feature_levels, nhead, enc_n_points, add_channel_attention=add_channel_attention, use_deformable_box_attn=use_deformable_box_attn, box_attn_type=box_attn_type,lora_rank=lora_rank,lora_pos=lora_pos)
        else:
        ...
        if deformable_decoder:
            decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward,
                                                          ...
                                                          module_seq=module_seq,lora_rank=lora_rank,lora_pos=lora_pos)
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, 
        ...
        if two_stage_type in ['enceachlayer', 'enclayer1']:
            _proj_layer = lora.Linear(d_model, d_model,r=(lora_rank if lora_pos=="FFN" else 0))
            _norm_layer = lora.LayerNorm(d_model)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, 
        ...
        self.ref_point_head = MLP(query_dim // 2 * d_model, d_model, d_model, 2,lora_rank=0,lora_pos="NO")
        if not deformable_decoder:
            self.query_pos_sine_scale = MLP(d_model, d_model, d_model, 2,lora_rank=lora_rank,lora_pos=lora_pos)
        else:
        ...
        if rm_dec_query_scale:
            ...
            raise NotImplementedError
            self.query_scale = MLP(d_model, d_model, d_model, 2,lora_rank=lora_rank,lora_pos=lora_pos)
        ...
        if not deformable_decoder and modulate_hw_attn:
            self.ref_anchor_head = MLP(d_model, d_model, 2, 2,lora_rank=lora_rank,lora_pos=lora_pos)
        else:
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        # ffn
        self.linear1 = lora.Linear(d_model, d_ffn,r=(lora_rank if lora_pos=="FFN" else 0))
        self.activation = _get_activation_fn(activation, d_model=d_ffn)
        self.dropout2 = nn.Dropout(dropout)
        self.linear2 = lora.Linear(d_ffn, d_model,r=(lora_rank if lora_pos=="FFN" else 0))
