...
def main(args):
    ...
    dataset_train_remain_merge = ConcatDataset([dataset_train_remain, dataset_train_learn])
    if args.distributed:
        # pass
        sampler_train_remain_merge = DistributedSampler(dataset_train_remain_merge)
        sampler_val_remain_merge = DistributedSampler(dataset_train_remain_merge, shuffle=False)

    else:
        sampler_train_remain_merge = torch.utils.data.RandomSampler(dataset_train_remain_merge)
        sampler_val_remain_merge = torch.utils.data.SequentialSampler(dataset_train_remain_merge)


    batch_sampler_train_remain_merge = torch.utils.data.BatchSampler(sampler_train_remain_merge,
                                                                     args.batch_size, drop_last=True)
    data_loader_train_remain_merge = DataLoader(dataset_train_remain_merge,
                                                batch_sampler=batch_sampler_train_remain_merge,
                                                collate_fn=utils.collate_fn, num_workers=args.num_workers)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.distributed:
            # pass
            sampler_train_remain_merge.set_epoch(epoch)
            sampler_train_forget.set_epoch(epoch)
        train_stats = train_one_epoch_forget_twice(
            model, criterion, data_loader_train_remain_merge,data_loader_train_forget, optimizer, device, epoch,
            args.clip_max_norm, wo_class_error=wo_class_error, lr_scheduler=lr_scheduler, args=args,
            logger=(logger if args.save_log else None), ema_m=ema_m)
        if args.output_dir:
            checkpoint_paths = [output_dir / 'checkpoint.pth']
        ...
        if args.output_dir:
            ...
            # extra checkpoint before LR drop and every 100 epochs
            if (epoch + 1) % args.lr_drop == 0 or (epoch + 1) % args.save_checkpoint_interval == 0:
                checkpoint_paths.append(output_dir / f'checkpoint{epoch:04}_forget_twice.pth')
            for checkpoint_path in checkpoint_paths:
                weights = {
                    'model': model_without_ddp.state_dict(),
                    'optimizer': optimizer.state_dict(),
