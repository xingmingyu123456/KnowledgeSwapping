...
class CosFace(nn.Module):
    ...
    def __init__(self, in_features, out_features, device_id, s=64.0, m=0.35):
        ...
        )  # equivalent to self.fc = nn.Linear(in_features, out_features)
        nn.init.xavier_uniform_(self.weight)
...
class SFaceLoss(nn.Module):
    ...
    def __init__(
        self, in_features, out_features, device_id, s=64.0, k=80.0, a=0.80, b=1.22
    ):
...
class ViT_face_low(nn.Module):
    """
    ViT_face_low:
    Used for feature extraction from the first half ViT, returns the features of layer depth//2
    """
...
class ViT_face_up(nn.Module):
    """
    ViT_face_up: Used for feature extraction from the second half of ViT, returning the final feature
    """
