...
class Attention(nn.Module):
    def __init__(self, config, vis,lora_rank):
...
class Mlp(nn.Module):
    def __init__(self, config,lora_rank=8):
        super(Mlp, self).__init__()
        self.fc1 = lora.Linear(config.hidden_size, config.transformer["mlp_dim"],r=lora_rank)
        self.fc2 = lora.Linear(config.transformer["mlp_dim"], config.hidden_size,r=lora_rank)
...
class Block(nn.Module):
    def __init__(self, config, vis,lora_rank,lora_pos):
        super(Block, self).__init__()
        self.hidden_size = config.hidden_size
        self.attention_norm = LayerNorm(config.hidden_size, eps=1e-6)
        self.ffn_norm = LayerNorm(config.hidden_size, eps=1e-6)
        self.ffn = Mlp(config,lora_rank=(
        lora_rank if lora_pos == "FFN" else 0
                                    ))
        self.attn = Attention(config, vis,lora_rank=(
...
class Encoder(nn.Module):
    def __init__(self, config, vis,lora_rank,lora_pos):
        ...
        for _ in range(config.transformer["num_layers"]):
            layer = Block(config, vis,lora_rank,lora_pos)
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, vis,lora_rank,lora_pos):
        super(Transformer, self).__init__()
        self.embeddings = Embeddings(config, img_size=img_size)
        self.encoder = Encoder(config, vis,lora_rank,lora_pos)
...
class ViT_bird(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False,lora_rank=8,lora_pos:str="FFN"):
        super(ViT_bird, self).__init__()
        self.num_classes = num_classes
        self.zero_head = zero_head
        self.classifier = config.classifier

        self.transformer = Transformer(config, img_size, vis,lora_rank,lora_pos)
        self.head = Linear(config.hidden_size, num_classes)

    def forward(self, x, labels=None):
        x, attn_weights = self.transformer(x)
        logits = self.head(x[:, 0]) # 就是第一位和第二位
