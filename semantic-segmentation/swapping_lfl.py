...
class Trainer_forget(DefaultTrainer):
    ...
    def run_step(self):
        ...
        if self.iter % 2000 == 1000 and self.iter != 0:
            self.custom_save(f"forget_model_{self.iter:07d}")
        if self.iter % 1000 == 0 and self.iter != 0:
            for key, val in self.grad_dic.items():
                self.grad_dic[key] = val / self.iter
...
class Trainer_learn_twice(DefaultTrainer):
    """
    Extension of the Trainer class adapted to MaskFormer.
    """
    def __init__(self, cfg):
    ...
    def run_step(self):
        ...
        if self.iter % 2000 == 1000 and self.iter != 0:
            self.custom_save(f"learn_twice_model_{self.current_iter:07d}")
...
def learn_twice(args):
    ...
    if args.eval_only:
        ...
        # )
        trainer.model.eval()
        res = Trainer_learn_twice.test(cfg, trainer.model)
        # if cfg.TEST.AUG.ENABLED:
        #     res.update(Trainer_learn.test_with_TTA(cfg, model))
        if comm.is_main_process():
            verify_results(cfg, res)
    ...
    trainer = Trainer_learn_twice(cfg)
    # args.resume = True
    # trainer.resume_or_load(resume=args.resume)
    trainer.custom_load(cfg.MODEL.WEIGHTS)
    if cfg.MODEL.SEM_SEG_HEAD.LORA_RANK > 0:
